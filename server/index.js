...
const { fetchMaserverUserData } = require('./auth/maserver');
...
apiRouter.post('/login', async (req, res) => {
  try {
    ...
    const maserverResult = await fetchMaserverUserData(log, pwd);
    if (!maserverResult.success) {
      return res.status(404).json({ message: maserverResult.message });
    }
